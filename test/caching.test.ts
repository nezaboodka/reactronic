...
test("caching", t => {
  ...
  t.is(demo.extra, "updated extra")
  t.assert(demo.title.startsWith("demo -")) // check that Demo.normalizeTitle works
  t.throws(() => demo.produceSideEffect(), { message: "Demo.produceSideEffect #22 should not have side effects (trying to change Demo.title #22t105s102)" })
  // t.throws(() => demo.setUninitialized('someValue'), { message: 'uninitialized member is detected: t107s103#21 Demo.uninitialized' })
  t.assert(demo.raw.startsWith("Demo.produceSideEffect #22t105s102   ◀◀   T105[Demo.produceSideEffect #22]"))
