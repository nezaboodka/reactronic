...
test('brief', t => {
  ...
  try {
    t.is(R.why(), 'N/A')
    t.is(R.getMethodCacheState(app.print).options.priority, 123)
    t.notThrows(() => DemoView.test())
    const rendering = R.getMethodCacheState(app.render)
    ...
    t.throws(() => { R.getMethodCacheState(daddy.setParent).configure({ monitor: null }) }, { message: 'given method is not decorated as reactronic one: setParent' })
    t.throws(() => { console.log(R.getMethodCacheState(daddy.setParent).options.monitor) }, { message: 'given method is not decorated as reactronic one: setParent' })
