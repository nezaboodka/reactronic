...
// Copyright (C) 2016-2026 Nezaboodka Software <contact@nezaboodka.by>
// License: https://raw.githubusercontent.com/nezaboodka/reactronic/master/LICENSE
// By contributing, you agree that your contributions will be
// automatically licensed under the license referred above.

import test, { ExecutionContext } from "ava"
import { runTransactional, all, pause, Reentrance, ReactiveSystem, manageReaction, disposeRxObject } from "../source/api.js"
...
test("reentrance.cancel", async (t: ExecutionContext<unknown>) => {
  ReactiveSystem.setLoggingMode(true, TestsLoggingLevel)
  const app = runTransactional(() => {
    const a = new AsyncDemoView(new AsyncDemo())
    manageReaction(a.print).configure({ order: 0 })
    manageReaction(a.model.load).configure({reentrance: Reentrance.cancelPrevious})
    return a
  })
  try {
