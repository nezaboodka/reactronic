...
import { ReactiveSystem, options, unobservable, reaction, nonreactive, impact } from "../ReactiveSystem.js"
...
class ReactiveNodeImpl<E = unknown> extends ReactiveNode<E> {
  ...
  static setNodeVariableValue<T extends Object>(variable: ReactiveNodeVariable<T>, value: T | undefined): void {
    ...
    if (value && value !== hostCtx) {
      ...
      impact({ isolation: Isolation.joinAsNestedTransaction }, () => {
  ...
  if (node.stamp >= 0) {
    ...
    if (node.has(Mode.autonomous)) {
      ...
      if (gFirstToDispose === slot)
        impact({ isolation: Isolation.disjoinForInternalDisposal, hint: `runDisposalLoop(initiator=${slot.instance.key})` }, () => {
