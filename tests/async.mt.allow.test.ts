...
import { ReactiveCache, Transaction, ConcurrentCall, Debug } from "../src/z.index";
...
let etalon: string[] = [
  ...
  "Url: nezaboodka.com",
  "Log: RTA, nezaboodka.com/100",
  "Error: [E604] transaction t32'recache conflicts with other transactions on: t26#22 DemoModel.url, t26#22 DemoModel.log",
];

test("async", async t => {
  Debug.verbosity = process.env.AVA_DEBUG === undefined ? 0 : 2;
  let app = Transaction.run(() => new DemoView(new DemoModel()));
  app.model.load.rcache.configure({concurrentCall: ConcurrentCall.RunSimultaneously});
  try {
